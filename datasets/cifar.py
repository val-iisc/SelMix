...
def get_transform(mean, std, train=True, size=32):
    ...
    Args:
        mean: Mean values for data normalization.
        std: Standard deviation values for data normalization.
        train: True for training transformations, False for testing transformations.
        size: Size of the image after transformations.
...
class CIFAR_SSL_LT_Dataset:
    """
    SSL_Dataset class for handling labeled and unlabeled datasets with long-tail distribution.
    """
    def __init__(self, name='cifar100', num_classes=100, data_dir='./data',
        ...
        # Define mean and standard deviation values for data normalization
    ...
    def get_data(self):
        """
        Load and preprocess the dataset to get the 
        training(labeled U unlabeled) set, validation set 
        and testing set
        """
    ...
    def resize(self, data, targets):
        """
        Resize the dataset into training (labeled U unlabeled set) and 
        ...
        Returns:
            Resized data and targets for training and validation.
        """
        ...
        # Select remaining samples for validation
        ...
        # Populate the target index dictionary
        for idx, target in zip(val_idx, val_targets):
            target_idx_dict[target].append(idx)

        # Sort the validation indices based on target classes
        val_idx_ = []
        for i in range(self.num_classes):
            val_idx_ = val_idx_ + target_idx_dict[i]
        val_idx = val_idx_

        # Select validation data and targets
    ...
    def apply_longtail_sampling(self):
        ...
        # Update the labeled data and targets based on the selected samples
        ...
        # Update the unlabeled data and targets based on the selected samples
