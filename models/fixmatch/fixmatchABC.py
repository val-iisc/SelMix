...
class FixMatch:
    def __init__(self, net_builder, num_classes, ema_m, T, p_cutoff, lambda_u,\
                 hard_label=True, t_fn=None, p_fn=None, it=0, num_eval_iter=1000, tb_log=None, logger=None, classes=None):
        """
    ...
    def abc_mask(self, gt):
        y = torch.clone(gt)
        y_ = y.detach().cpu().numpy().tolist()
        mask = []
        for y1 in y_:
            lambda_ = 1 - (self.it/self.args.num_train_iter) * (1 - self.prior[-1]/self.prior[y1])
            mask.append(np.random.binomial(1, lambda_))
        mask = torch.tensor(mask).cuda(self.args.gpu)
        return mask
    def train(self, args, logger=None):
        ...
        for (x_lb, y_lb), (x_ulb_w, x_ulb_s, _) in\
            ...
            with amp_cm():
                logits_x_lb, logits_x_lb_bal, logits_x_ulb_w, logits_x_ulb_w_bal, logits_x_ulb_s, logits_x_ulb_s_bal = self.train_model(x_lb, x_ulb_w, x_ulb_s)
                ...
                sup_loss_abc = torch.mean(self.abc_mask(y_lb) * F.cross_entropy(logits_x_lb_bal,\
                                                                        y_lb,\
                                                                        reduce=False,\
                                                                        reduction="None"))
                
                y_pl = torch.argmax(logits_x_ulb_w_bal, dim=-1)
                abc_mask_consistency = self.abc_mask(y_pl)
                unsup_loss_abc_masked, mask = consistency_loss(logits_x_ulb_w_bal, 
                                                    logits_x_ulb_s_bal, 
                                                    'ce', T, p_cutoff,
                                                    use_hard_labels=args.hard_label,
                                                    reduce=False)
                ...
                unsup_loss_masked_abc = torch.mean(unsup_loss_abc_masked * abc_mask_consistency)
                total_loss = (sup_loss + self.lambda_u * unsup_loss)\
                            + (self.lambda_u * unsup_loss_masked_abc + sup_loss_abc)
