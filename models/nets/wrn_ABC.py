...
class WideResNet(nn.Module):
    def __init__(self, depth, num_classes, widen_factor=1, bn_momentum=0.1, leaky_slope=0.0, dropRate=0.0):
        ...
        self.fc_bal = nn.Linear(nChannels[3], num_classes)
    ...
    def forward(self, x, u_w, u_s):
        x_feats, u_w_feats, u_s_feats = self.get_feats(x), self.get_feats(u_w) , self.get_feats(u_s)
        return self.fc(x_feats), self.fc_bal(x_feats), self.fc(u_w_feats), self.fc_bal(u_w_feats), self.fc(u_s_feats), self.fc_bal(u_s_feats)
        
    def infer(self, x):
        feats_x = self.get_feats(x)
        return self.fc_bal(feats_x)
    
    def infer_feats(self, x):
        return self.fc_bal(x)
